...
(defcustom org-download-screenshot-method "nircmd.exe savescreenshot %s"
  "The tool to capture screenshots."
  :type '(choice
          (const :tag "gnome-screenshot" "gnome-screenshot -a -f %s")
          (const :tag "scrot" "scrot -s %s")
          (const :tag "nircmd.exe" "nircmd.exe savescreenshot %s")
...
(defun org-download--image (link filename)
  ...
  (message "link: %s, filename: %s" link filename)
...
COMMAND is a format-style string with two slots for LINK and FILENAME."
  (message "command:%s, link: %s, filename: %s" command link filename)
  (shell-command
   (format command link
...
The screenshot tool is determined by `org-download-screenshot-method'."
  (interactive)
  (let ((link "E:/home/tmp/screenshot.png"))
...
(defun org-download-file (link)
  "Save file at address LINK to `org-download--dir'."
  (interactive "sUrl: ")
  (message "hex link: " link)
  (let ((filename
         (if (eq org-download-method 'attach)
             (let ((org-download-image-dir (progn (require 'org-attach)
                                                  (org-attach-dir t)))
...
;; modify the open local file, if current mode is org-mode and the file is a image file, then insert it to the org file instead of open it.
    ...
    (if (and f (file-readable-p f))
        (progn
          ...
          (if (string-equal major-mode "org-mode")
              ;; (member-ignore-case (file-name-extension f) '("jpg" "png" "jpeg" "tiff"))
              (org-download-file f)
