...
;;----------------------------------------------------------------------------
;; mic-paren can't work in emacs 23.4,so disable it
;; And now 24.3 also doesn't work. So disable it always
;; (or *is-emacs23*
...
;; This key is confilic with org-capture
...
;; save os clipboard contents to kill ring before replacing them.
;; http://emacs.stackexchange.com/questions/766/add-operating-system-clipboard-to-kill-ring
...
(setq aspk/buffer-list nil)

(defun aspk/add-to-top-of-buffer-list (elm)
  (setq aspk/buffer-list (cons elm aspk/buffer-list)))

(defun aspk/delete-from-buffer-list (elm)
  (let ((tmp aspk/buffer-list)
        tmp1)
    (while tmp
      (if (string-equal elm (car tmp))
          (progn
            (setq tmp1 (cdr tmp))
            (setcar tmp (car tmp1))
            (setcdr tmp (cdr tmp1)))
...
(defun aspk/delete-from-buffer-list (elm)
  (setq aspk/buffer-list (delete elm aspk/buffer-list)))

(defun aspk/get-from-buffer-list (n)
  (nth n aspk/buffer-list))
...
;; test
;; (aspk/get-from-top-of-buffer-list)
;; (aspk/delete-from-buffer-list "init-misc.el")
...
;; (setq aspk/tmp nil)
;; (cdr aspk/tmp)
(defun aspk/forward-sexp (arg)
  "simple command testing (forward-sexp 1)"
  (interactive)
  (condition-case ee
      (forward-sexp arg)
    (scan-error
     (goto-char (+ 0 (nth 3 ee)))
     (message "aspk/forward-sexp failed: %S" ee))))

(require 'aspk-advice)
(aspk/advice-add 'eval-buffer 'before (lambda (&rest args)
                                        (add-pwd-into-load-path)))
(aspk/advice-add 'eval-buffer 'after (lambda (&rest args)
