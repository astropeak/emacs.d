...
with more keys."
  ...
  (or input-method-use-echo-area
      (let (pos)
        ...
        ;; instead of insert current str(which is a translated key), we insert the key
...
;; this variable controls how input is displayed
           ...
           (let* ((str (quail-get-current-str
                  ...
                  (str1 (and (quail-deterministic)
                             (quail-get-current-str
                              ...
                              (quail-map-definition (quail-lookup-key
                                                     substr1 1))))))
             (message "str:%s, substr1:%s" str substr1)
             (if str
  ...
  (let* ((len (length quail-current-key))
    ...
    (if map
        (let ((def (quail-map-definition map)))
          ...
          (message "quail-current-str: %s" quail-current-str)
...
(defun quail-input-current-key ()
  "Abort translation and insert the current Quail key sequence."
  (interactive)
  (dbg TRIV quail-current-key)
  (quail-delete-region)
  (insert quail-current-key)
  (setq quail-current-str nil)
  (quail-terminate-translation))
